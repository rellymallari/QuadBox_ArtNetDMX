...
#define SONOFF_DMX_CHANNEL 3 //(DMX Channels 1-512)
#define SONOFF_STATUS_LED 14
#define SONOFF_MIN_INTENSITY 5
#define SONOFF_ONOFF_BUTTON 0

uint32_t sonoffLastStatusUpdate = 0, sonoffLastButtonPress = 0;
bool sonoffRelayState = true;
...
void setup(void) {
  ...
  digitalWrite(SONOFF_RELAY_PIN, HIGH); //Relay ON
  pinMode(SONOFF_STATUS_LED, OUTPUT);
  digitalWrite(SONOFF_STATUS_LED, LOW);  //Status LED ON
  pinMode(SONOFF_ONOFF_BUTTON, INPUT);  //on-off button
...
void loop(void){
  ...
  // Handle Sonoff
  if (millis() - sonoffLastStatusUpdate > 2000) { // No ArtNet signal, port closed
    digitalWrite(SONOFF_STATUS_LED, LOW);  //Put status LED to constant on

    //Enable Sonoff On-off button
    // button pressed = low reading
    if (!digitalRead(SONOFF_ONOFF_BUTTON)){
      if(millis () - sonoffLastButtonPress > 50)
        if(sonoffRelayState) {
          digitalWrite(SONOFF_RELAY_PIN, LOW); //off
          sonoffRelayState = false;
        }
        else {
          digitalWrite(SONOFF_RELAY_PIN, HIGH); //on
          sonoffRelayState = true;
        }
      sonoffLastButtonPress = millis();
    }
  }
...
void dmxHandle(uint8_t group, uint8_t port, uint16_t numChans, bool syncEnabled) {
  if (portA[0] == group) {
    ...
    } else if (deviceSettings.portAmode == TYPE_RDM_OUT) {
        //Sonoff
        byte* a = artRDM.getDMX(group, port);
        if(a != NULL){
          if(a[SONOFF_DMX_CHANNEL - 1] >= SONOFF_MIN_INTENSITY){
            digitalWrite(SONOFF_RELAY_PIN, HIGH);  //on
          }
          else {
            digitalWrite(SONOFF_RELAY_PIN, LOW);  //off
          ...
          if (statusTimer < millis()) {
            if ((statusTimer % 1000) > 500)
              digitalWrite(SONOFF_STATUS_LED, HIGH); //off
            else
              digitalWrite(SONOFF_STATUS_LED, LOW);  //on
            statusTimer = millis() + 500; 
            sonoffLastStatusUpdate = millis();
          }     
        }
